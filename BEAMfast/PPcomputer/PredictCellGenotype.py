...
class PredictCellGenotype():
    def __init__(self, id0, seqs, PPcut, OutTreeFile): #seqs is list format of mega alignment
      self.PPcut=PPcut	  
      Align = MegaAlignment()
      tree_builder = MegaML() 
      tree_analyzer = TreeAnalizer()  
      self.InMeg = seqs#Align.AddNormal(seqs)
      if os.path.exists(OutTreeFile)!=True: 
        # status = tree_builder.do_mega_ml(self.InMeg, id0)
        # if status == True:
        #       tree = tree_builder.newick_trees
        #       OutF=open(OutTreeFile,'w')
        #       OutF.write(tree)
        #       OutF.close()
        # else:
                print('please provide tree',OutTreeFile)
      else: 
          tree=open(OutTreeFile,'r').readlines()[0]                
          self.Tree_rooted = tree#open(tree,'r').readlines()[0]#tree_analyzer.RootTree(tree, 'Normal')
